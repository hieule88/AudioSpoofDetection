...
datadir=/home/hieuld/workspace/ASVspoof2019

stage=0

. ./parse_options.sh || exit 1

mkdir -p data/PA_train data/PA_dev data/PA_eval  || exit 1

if [ $stage -le 0 ]; then
   echo "Stage 0: prepare dataset."
   for access_type in PA ;do
       protofile=$datadir/${access_type}/ASVspoof2019_${access_type}_cm_protocols/TrainFinetune.txt
       ...
       awk -v dir="${datadir}/${access_type}" -v type="${access_type}_train" '{print $2" sox "dir"/train/"$2".flac -t wav - |"}' $protofile >data/${access_type}_train/wav.scp || exit 1

       for dataset in dev eval ;do
           protofile=$datadir/${access_type}/ASVspoof2019_${access_type}_cm_protocols/TestFinetune.txt
           ...
           awk -v dir="${datadir}/${access_type}" -v type="${access_type}_${dataset}" '{print $2" sox "dir"/testfinetune/"$2".flac -t wav - |"}' $protofile >data/${access_type}_${dataset}/wav.scp || exit 1
       done
    done
...
if [ $stage -le 1 ]; then
   ...
   for name in PA_train PA_dev PA_eval ; do
       [ -d data/spec/${name} ] || cp -r data/${name} data/spec/${name} || exit 1
       feats_extraction/make_spectrogram.sh --feats-config conf/feats/spec.conf --nj 30 --cmd "$train_cmd" \
...
if [ $stage -le 2 ]; then
   ...
   python3 feats_extraction/compute_CQT.py --out_dir data/cqt --access_type PA --param_json_path conf/feats/cqt_48bpo_fmin15.json --num_workers 1 || exit 1
