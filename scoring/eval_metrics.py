...
def obtain_asv_error_rates(tar_asv, non_asv, spoof_asv, asv_threshold):
    ...
    Pfa_asv = 1. * sum(non_asv >= asv_threshold) / non_asv.size
    Pmiss_asv = 1. * sum(tar_asv < asv_threshold) / tar_asv.size

    # Rate of rejecting spoofs in ASV
    if spoof_asv.size == 0:
        Pmiss_spoof_asv = None
    else:
        Pmiss_spoof_asv = 1. * np.sum(spoof_asv < asv_threshold) / spoof_asv.size
