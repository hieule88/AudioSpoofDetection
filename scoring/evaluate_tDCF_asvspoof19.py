...
# Replace ASV scores with organizers' scores or provide score file as the first argument.
asv_score_file = 'asv_scores/pa_scores'
# Replace CM scores with your own scores or provide score file as the second argument.
cm_score_file = 'scores/cm_dev.txt'

args = sys.argv
if len(args) > 1:
    if len(args) != 3:
        print('USAGE: python evaluate_tDCF_asvspoof19.py <ASV_SCOREFILE> <CM_SCOREFILE> ')
        exit()
    else:
        asv_score_file = args[1]
        cm_score_file = args[2]
...
eer_cm, cm_thresh = em.compute_eer(bona_cm, spoof_cm)
...
#         num_bona = num_bona +1
# print('DEBUGGGGGGG:')
# print(1-(num_bona/len(bona_cm)))
# num =0
# for i in spoof_cm:
#     if i < cm_thresh:
#         num = num +1
# print(1-(num/len(spoof_cm)))

# print(1 - ((num + num_bona)/(len(bona_cm)+len(spoof_cm))))
# print(cm_thresh)
